...
interface SyncedScrollViewProps extends ScrollViewProps {
  currentScrollViewId: number
}

export const SyncedScrollView = (props: SyncedScrollViewProps) => {
  const { currentScrollViewId, ...rest } = props;
  ...
  offsetPercent?.addListener(({ value }) => {
    ...
    // @ts-ignore
    if (currentScrollViewId !== activeScrollView._value && scrollableLength > 0) {
  ...
  const handleTouchStart = () => {
    activeScrollView.setValue(currentScrollViewId)
  }
